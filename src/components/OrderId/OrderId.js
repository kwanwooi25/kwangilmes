...
class OrderId extends Component {
	...
	renderFields = () => {
		...
		return FIELDS.map(({ varName, displayName }) => {
			...
			switch (varName) {
				case 'ordered_at':
				case 'completed_at':
					if (order[varName]) value = moment(order[varName]).format('YYYY-MM-DD');
					else value = '';
					break;
				...
				case 'print_info':
					const { ext_color, is_print, print_front_color_count, print_back_color_count } = order;
					value = `${ext_color}원단`;
					if (is_print) {
						const colorCount = Number(print_front_color_count) + Number(print_back_color_count);
	...
	render() {
		...
		return (
			<div>
				<span className={`order_id ${className}`} onClick={this.showDetailView.bind(this)}>
					{order.id}
				</span>
