...
class ProductOrderForm extends Component {
  ...
  onConfirmModalClose = result => {
    ...
    if (result) {
      const data = {
        ...
        ordered_at: moment(this.state.ordered_at).format('YYYY-MM-DD'),
        deliver_by: moment(this.state.deliver_by).format('YYYY-MM-DD'),
  ...
  onClickOk = () => {
    if (this.validate()) {
      ...
      if (order_memo_delivery !== '')
        orderDetailArray.push(order_memo_delivery);
