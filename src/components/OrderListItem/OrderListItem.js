	...
	return (
		<li key={id} className={className}>
			...
			<Grid container spacing={8} className="list-body__item-details">
				<Grid item xs={6} lg={2}>
					<Grid item xs={12} className="order-list-item__delivery-remark-wrapper">
						{is_delivery_strict && <span className="order-list-item__delivery-remark">납기엄수</span>}
						{is_urgent && <span className="order-list-item__delivery-remark">지급</span>}
						{is_order_modified && <span className="order-list-item__delivery-remark modified">수정됨</span>}
						{is_completed && <span className="order-list-item__delivery-remark completed">완료</span>}
					</Grid>
					<Grid item xs={12}>
						<OrderId order={order} />
					</Grid>
				...
				<Grid item xs={6} lg={2}>
					<Grid item xs={12} className="order-list-item__dates-wrapper">
						<span className="order-list-item__dates">발주일: {moment(ordered_at).format('YYYY-MM-DD')}</span>
					</Grid>
					<Grid item xs={12} className="order-list-item__dates-wrapper">
						<span className="order-list-item__dates">납기일: {moment(deliver_by).format('YYYY-MM-DD')}</span>
					</Grid>
					{is_completed && (
						<Grid item xs={12} className="order-list-item__dates-wrapper">
				...
				<Grid item xs={6} sm={3} lg={3}>
					<Grid item xs={12} className="order-list-item__quantity-wrapper">
						...
						<span className="order-list-item__quantity">{comma(order_quantity)}매</span>
					...
					{completed_quantity && (
						<Grid item xs={12} className="order-list-item__quantity-wrapper">
							...
							<span className="order-list-item__quantity">{comma(completed_quantity)}매</span>
							<span className="order-list-item__quantity-weight">
